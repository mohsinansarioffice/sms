...
const ExamResults = () => {
  ...
  const { fetchExamResults, enterMarks, currentExam, examResults, isLoading } =
    useExamStore();

  const [students, setStudents] = useState([]);
  const [marksData, setMarksData] = useState({});
  /** One loader for the initial exam + student roster chain (avoids a double “loading” moment). */
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const examRes = await fetchExamResults(id, { silent: true });
      if (cancelled) return;
      ...
      const exam = useExamStore.getState().currentExam;
      if (!exam) {
        setIsPageLoading(false);
        return;
      }
      ...
      try {
        ...
        if (exam.classId?.name) params.append("class", exam.classId.name);
        if (exam.sectionId?.name) params.append("section", exam.sectionId.name);
    ...
    if (students.length > 0 && currentExam) {
      ...
      students.forEach((student) => {
        const existingResult = examResults.find(
          (r) => (r.studentId?._id || r.studentId) === student._id,
        );
        if (existingResult) {
  ...
  const onSubmit = async () => {
    const resultsPayload = Object.entries(marksData).map(
      ([studentId, data]) => {
        return {
          studentId,
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        {currentExam ? (
          <p className="text-sm text-gray-600 mb-4">
            ...
            {currentExam.subjectId?.name} | Max: {currentExam.totalMarks} |
            Pass: {currentExam.passingMarks}
          </p>
        ) : null}
        <div className="card p-0 overflow-hidden shadow-sm">
          {isPageLoading ? (
            <div className="p-12 text-center">
              ...
              <p className="text-gray-500">Loading exam and student roster…</p>
            </div>
          ) : students.length === 0 ? (
                      ...
                      <tr key={student._id} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-center">
                          <label className="inline-flex items-center gap-2 cursor-pointer">
                            ...
                            />
                            <span
                              className={`text-sm font-medium ${data.isAbsent ? "text-red-600" : "text-gray-500"}`}
                            >
                              Absent
                            </span>
                          </label>
